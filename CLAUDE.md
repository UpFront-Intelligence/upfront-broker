...
- **Known source data quality issue — brand null on some chains:** `brand.names.primary` is null in Overture for certain branded locations including some Western Union outlets, Allstate franchise locations, and similar. This is an Overture data quality issue, not a script bug — `TRY(brand.names.primary)` extracts correctly, the field is genuinely absent in Overture's source data for those rows. `names.primary` (the location name) is always populated and serves as the display fallback.
