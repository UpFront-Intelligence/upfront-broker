...
- **Current migration head:** `f112218c13fe` (`add_property_parties_reverse_lookup_indexes`) — 27 migrations total, linear chain, no branches. This doc had already silently drifted one migration behind (`97ff9ec4ed1c`/`add_parcels_regrid_searchable_fields` existed undocumented) before this entry — verify with `alembic heads` if it drifts again rather than trusting this line.
...
- **Frontend typeahead reuses the existing `/accounts/search` and `/contacts/search` endpoints** (the same ones the Property detail page's Recorded Owner "Change" flow already uses) — called in parallel and merged client-side into one dropdown via the shared `Typeahead` module (`frontend/js/app.js`), tagging each hit with a `meta` badge ("Account"/"Contact"). No new search endpoint was added. The typeahead also offers `+ Create "{q}" as new Account` (mirrors the Recorded Owner flow's `createOwnerAccount()` exactly — `POST /accounts/` then proceed immediately) and `+ Create "{q}" as new Contact` (chains three existing endpoints client-side: `POST /accounts/` → `POST /contacts/` → `POST /accounts/{account_id}/contacts` to link them — the last of these pre-existed in `accounts.py` with zero frontend callers until now). Both create options auto-select the new entity and immediately submit the party (via `submitAddParty()`) with whatever role is already chosen in the dropdown — no re-search required.
- **"Every Contact should have an Account" is enforced ONLY in this flow's "+ Create Contact" option — a known, deliberately tracked inconsistency, not a codebase-wide rule.** `createPartyContact()` always creates (or would need to create) a matching Account for a new Contact — e.g. a solo contact "Jane Smith" gets an Account also named "Jane Smith" when no distinct company applies — but this guarantee does **not** hold anywhere else a Contact can be created:
  - `contacts.html`'s direct "add contact" modal (`saveContact()`) has no Account field at all and happily creates a fully orphan Contact — this is today's actual behavior, unchanged by this feature.
  - The general importer's party fan-out (`import_properties_parties.py`'s `_process_party()`) does the opposite of defaulting: `if not name: return` skips Contact creation entirely when no company-name column is present, rather than naming the Account after the contact.
  - Retrofitting the rule onto either of those is out of scope here and tracked as separate follow-up work — the importer change in particular could alter existing CSV import behavior (rows that today produce no Contact would start producing one plus a new Account) and needs its own careful review, not a quick fix.
- **Reverse lookup — Contact/Account detail pages (2026-07-22).** Until now `property_parties` was only ever queried from the property side (the Parties card above); a contact or account that's a party on many properties (e.g. a leasing broker) showed nothing related to them on their own detail page. `services/property_parties.py`'s `get_properties_for_party(db, owner_id, account_id=None, contact_id=None)` is the shared reverse resolver — one property per row even if the same party holds more than one role on it (role is part of the table's uniqueness key, not a single-value column), each row's `roles` list resolved against `account_roles` the same way `_resolve_parties()` already does property-side. Wired into `get_contact_full()`/`get_account_full()` as a new `associated_properties` response key (replaces the old `linked_properties` key on both, and `properties`/`owned_properties`/`managed_properties` on Account — none of the four survive on these two endpoints; `routers/query.py`'s own `_linked_properties_for_contact/_account()`, used separately by the unified Search/Map page's per-row map pins, are untouched).
  - **Account-side merge:** `get_account_full()` additionally merges in the three direct FK links (`account_id`, `recorded_owner_account_id`, `manager_account_id`) as synthetic role tags on the same `associated_properties` list, keyed/deduped by property id — `recorded_owner_account_id` tags as the real `owner` slug and `manager_account_id` as the real `property_manager` slug (so an FK-sourced tag dedupes cleanly against a `property_parties` row for the same role, and both pick up the existing `--role-owner`/`--role-manager` badge colors for free); plain `account_id` (no formal role in this schema) gets a synthetic `linked` tag with no category, which the frontend's `.role-badge[data-role]` CSS has no rule for and correctly falls back to its default grey — intentional, not a gap. `property_parties_count` (already consumed by `Hints._card()`'s duplicate-comparison popup in `app.js`) is now `len(associated_properties)` — the deduped merged total — instead of a raw `property_parties`-only row count; it had been computed and left unused by account.html until this change.
  - **New indexes** (migration `f112218c13fe`): `idx_pp_account_id`/`idx_pp_contact_id`, partial (`WHERE ... IS NOT NULL`, matching this table's existing partial-index style) — the three pre-existing indexes all lead with `property_id` and can't serve a `WHERE account_id = X`/`WHERE contact_id = X` lookup with no `property_id` in the filter; without these it's a full table scan.
  - **Frontend:** new shared `AssociatedProperties` module in `app.js` (`.render()` for the list + portfolio-count summary, `.pins()` for map markers) — used identically by both `contact.html` (new "Associated Properties" card, didn't exist before) and `account.html` (its existing "Properties" card was replaced by this, not added alongside it). Bucketing: a property tagged `owner` is "Owned" even if it also carries a brokerage_mgmt role on the same row (one section per property, not one per role — same "one signal, not stacked" precedent as Property Finder's `--owned-marker` override); anything with a brokerage_mgmt role and no `owner` tag is "Leasing / Brokerage"; everything else (buyer/tenant/legal/vendor roles, or the synthetic `linked` tag alone) falls to an "Other" catch-all rather than vanishing. The existing collapsed-by-default Map card on both pages now auto-expands on load whenever there's an office pin or at least one property with coordinates (still collapsible after); pins are colored by role priority (`owner` > `sale_broker` > `leasing_broker` > `tenant_rep` > `sublease_broker` > `property_manager` > default `--accent`) via the same `--role-*` tokens the badges use, and are now click-through to `property.html?id=X` (previously tooltip-only). Properties missing `lat`/`lng` (~8% per the Nominatim geocode gap noted elsewhere in this doc) still render in the list — coordinates aren't needed there — and are silently excluded only from the map layer, same pattern as everywhere else in this app.
...
| Contact detail | ✅ | Phones list (label · number, primary badge, inline edit); role-aware Associated Properties card (reverse `property_parties` lookup, see PROPERTY_PARTIES) with portfolio-count summary; office-location + associated-properties mini-map, auto-expanded when populated |
| Account detail | ✅ | Party hub: role badges, engagements, contacts; role-aware Associated Properties card merging `property_parties` with the three direct FK links (see PROPERTY_PARTIES); office-location + associated-properties mini-map, auto-expanded when populated |
