...
- **Current migration head:** `1323d6d9faea` (`add_national_locations`) — 24 migrations total, linear chain, no branches. Verify with `alembic heads` if this drifts.
...
### NATIONAL_LOCATIONS (Overture Maps Places — Michigan reference dataset)
```
...
- **Category scope (Overture 2026 top-level taxonomy — 11 total, we ingest 5):**
  - INCLUDED: `eat_and_drink`, `retail`, `automotive`, `beauty_and_spa`, `financial_service`
  - EXCLUDED: `accommodation`, `arts_and_entertainment`, `attractions_and_activities`, `active_life`, `education`, `private_establishments_and_corporates`
  - **Important discrepancy vs. original task spec:** the task listed `professional_services` and `health_and_medical` as top-level categories, but neither exists in Overture's actual 2026 taxonomy. Pharmacies (Walgreens, CVS) are under `retail.pharmacy`, correctly captured by the `retail` inclusion. Banks are under `financial_service`, not `professional_services`. Verify this mapping holds on the first real ingest and update the include list in `scripts/ingest_overture_michigan.py` (`INCLUDE_CATEGORIES` set) if needed.
- **`category_top`** is derived in Python (`categories.primary.split(".")[0]`), not stored from Overture directly — the full dot-path category (e.g. `eat_and_drink.restaurant.coffee_shop`) lives in `category_primary`; `category_top` is the first segment, used for browsable filtering in the Search UI.
...
- **Schema assumptions flagged for real-data verification** (built against docs before first production run, same posture as `parcels_regrid`):
  - `brand.names.primary` — confirmed from JSON shape `{"names": {"primary": "Starbucks"}, "wikidata": null}` in doc examples.
  - `addresses[1].{freeform,locality,region,postcode,country}` — confirmed from doc examples; 1-indexed in DuckDB SQL.
  - `bbox.xmin`/`bbox.ymin` — confirmed 2026 schema (older alpha releases used `bbox.minx`/`bbox.miny`; fixed in production schema).
  - `websites`/`phones` column types — may be JSON strings or native arrays depending on DuckDB version; script handles both.
...
- Created by `POST /api/national-locations/link-to-my-properties` — exact normalized-address match (`normalize_address()` from `services/naming.py`, same function the property importer uses) between `national_locations.address` and `properties.address`, grouped by `(city, state)` to limit comparison scope. Match confidence is 1.0 for exact normalized-address hits.
- **The "of 247 Walgreens in Michigan, you have 3" moment** — after running link-to-my-properties, the Search/Map "Locations" view shows an "In book ✓" badge on any location that's been linked to one of the broker's properties.
...
| **Search** | ✅ | Unified map + filterable list across Properties/Accounts/Contacts/Tenants/Deals/Locations (national_locations — amber pins, brand+category filters, "In your book" badge); collapsible map, hover/click pin-sync, debounced live filtering via `/api/query` |
